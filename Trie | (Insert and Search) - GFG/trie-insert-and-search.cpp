...
bool containsKey(TrieNode *node,char ch)
{
...
void putKey(TrieNode *node,char ch)
{
    ...
    node->children[ch-'a'] = nnode;
}


TrieNode *getNextNode(TrieNode *node,char ch)
{
    return node->children[ch-'a'];
}


void setEnd(TrieNode *node)
{
    node->isLeaf = true;
}
...
void insert(struct TrieNode *root, string key)
{
    ...
    for(int i=0;i<key.size();i++){
        if(!containsKey(node,key[i])){
            putKey(node,key[i]);
        }
        node = getNextNode(node,key[i]);
    }
